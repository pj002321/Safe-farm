 ...
 *
 * ★ 2026-09-20 — **과수는 `발아`·`개화` 가 온다.** 그 전까지 빈 값이던 자리다.
 *   과수는 나무를 몇 해 전에 심어서 그 해의 0일이 파종이 아니라 **기점**이다.
 *   그래서 `sow_from`·`sow_to` 가 가리키는 창도 파종 창이 아니라 **기점 창**이고,
 *   그 사실을 `sow_method` 값이 알려 준다(crop-data `build._파종방법`).
 *
 *   ⚠ **농민이 하는 일이 아니라 나무에 일어나는 일이다.** "심습니다" 로 뭉뚱그리면
 *     "3.15~4.5에 심습니다" 가 되어, 다 자란 사과나무를 가진 사람에게 틀린 말을 한다.
 *     기본값(`${method}을(를) 합니다`)에 맡기면 "발아을(를) 합니다" 로 조사도 틀린다.
 *
 *   ⚠ 과수는 `plant_from`·`plant_to` 가 비어 있어(실측) 모종 쪽 문장은 애초에 안 나온다.
 *
...
function toSowingPhrase(method: string | null): string {
  switch ((method ?? "").trim()) {
    ...
    case "인공수분":
      return "인공수분을 합니다";
    // 과수 — 사람이 하는 일이 아니라 나무에 일어나는 일이다 (위 ★)
    case "발아":
      return "싹이 틉니다";
    case "개화":
      return "꽃이 핍니다";
