...
class PlotTaskInputs:
    ...
    #: 나무를 심은 지 몇 해째인가. **과수만 찬다**(한해살이는 None).
    #:
    #: ⚠ 수확 카드를 막는 데만 쓴다(`과수수확중`). 1년차에는 열매가 안 달린다.
...
def 과수수확중(inputs: PlotTaskInputs) -> bool:
    ...
    if (inputs.sow_method or "").strip() not in _FRUIT_METHODS:
        return False
    # ⚠ **심은 해에는 열매가 안 달린다.** 사흘 전에 심은 단감 묘목이 `꽃눈분화기` 로
    #   판정되던 자리다(2026-09-21 실측) — 기점 되감기가 심기 5개월 전부터 열을
    #   쌓기 때문이다. 그 자체는 맞는 계산이다(옮겨 심어도 그해 생육은 이어진다).
    #   다만 **1년차 묘목**에 수확 카드를 보내면 틀린 말이 된다.
    #
    #   ⚠ 2년차 이후는 안 막는다. 작물별 **결실 시작 나이**(사과 3~5년 · 감귤 4~5년)가
    #     마스터에 없어 지어내게 된다. "심은 해엔 안 열린다" 까지만 확실하다
    #     (`이슈/과수_GDD모델의_한계_셋.md` ②).
    #   ⚠ 물·병해충·전정 카드는 그대로 나간다. 묘목에도 그 일들은 해야 한다.
    if inputs.years_since_planting == 1:
        return False
