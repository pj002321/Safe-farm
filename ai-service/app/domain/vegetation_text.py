...
def vegetation_lines(
    ...
    *,
    ...
    ⚠ **NDVI 가 음수면 통째로 침묵한다.** 음수는 물·건물·아스팔트라 그 좌표에
      작물이 없다는 뜻이고, 그러면 NDMI 추세도 뜻이 없다("물기가 늘었어요" 가
      아스팔트에 대해 나온다). 화면(`SatellitePanel`)에는 "밭 위치가 맞는지
      확인해 주세요" 를 **그대로 둔다** — 거기서는 쓸모 있는 말이다. 다만
      프롬프트에 넣으면 LLM 이 작물 조언 대신 좌표 얘기를 하고, 그 한 번의
      호출이 통째로 버려진다. 안 보내는 것이 "무시하라" 고 시키는 것보다 싸고
      확실하다 — 규칙이 사실을 정하고 LLM 은 엮기만 한다.
    """
    if v.ndvi is not None and v.ndvi < NDVI_NONE:
        return []
