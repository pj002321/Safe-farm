...
#
# ⚠ **실측이 아니다.** 위 RAIN_SOON_MM 과 같은 상식값인데, 그 표시가 없어 한동안
#   근거 있는 값인 척하고 있었다(2026-09-19 지적).
#
# ⚠ **흙 종류에 따라 뜻이 갈린다.** 이게 격자 크기보다 근본적인 한계다 —
#
#       모래흙  위조점 0.05~0.10  →  0.20 이면 아직 촉촉하다. 자주 발동한다
#       찰흙    위조점 0.20~0.27  →  0.20 에 영영 안 닿는다. 발동이 안 된다
#
#   우리는 그 밭 흙이 무엇인지 모른다. 같은 0.20 이 밭마다 다른 뜻이 된다.
#
# ⚠ 판정을 **혼자 뒤집지 못한다.** 모델값이라 믿을 수 있는 만큼만 쓴다 —
#   수지가 이미 마른 쪽일 때 등급을 올리는 데만 거든다(아래 `give` 참고).
#   값이 없으면(None) 판정에서 빠진다. 없는 것과 0.0 은 다르다.
#
# 한 계절 돌려 보고 **발동 빈도를 센다.** 찰흙 밭에서 영영 안 뜨면 그때 흙 종류를
# 받든지 이 규칙을 떼든지 정한다. 실측(2026-09-19 · 밭 7곳): 0.221~0.302 로
# 기준 언저리였다 — 닿을 만한 거리다.
SOIL_DRY = 0.20
