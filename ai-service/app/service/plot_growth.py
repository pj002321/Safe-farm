...
def compute_plot_growth(db: Session, plot: Plot, station: Station) -> PlotGrowth | None:
    """파종일부터 오늘까지 GDD 를 누적해 현재 생육단계를 계산한다.

    파종일을 모르거나(sowing_unknown), 작물 마스터 데이터가 없거나(crops.csv 미등재
    — 단감 등 과수), 품종 정보가 없으면 None.
    """
    if not plot.sowing_date or plot.sowing_unknown or not plot.crops:
        return None
    crop_name_ko = CROP_ID_TO_NAME_KO.get(plot.crops[0])
    if crop_name_ko is None:
        return None
    ...
    return PlotGrowth(
        ...
        days_since_planting=(date.today() - plot.sowing_date).days,
