...
# ── 이맘때 미리 해 둘 것 (disaster_bulletins) ────────────────────
#
# ★ 1,881행이 적재돼 있는데 꺼내 쓰는 코드가 적재 스크립트뿐이었다.
#   벡터로는 614건이 들어가 질문답변에는 잡히지만, 리포트는 벡터를 안 본다.


def test_이맘때_대비를_프롬프트에_싣는다():
    prompt = 밭(prevention_notes=("집중호우: 배수구를 깊게 정비한다",))
    assert "이맘때 미리 해 두는 것" in prompt
    assert "배수구" in prompt


def test_지금_온다는_뜻으로_쓰지_말라고_못박는다():
    """2023년 이후 자료라 올해 그 재해가 온다는 뜻이 아니다.
    단정하면 거짓이 되고, 한 번 틀린 경보를 보면 맞는 경보도 무시하게 된다."""
    prompt = 밭(prevention_notes=("폭염: 멀칭재배 실시",))
    assert "지금 그 재해가 온다는 뜻으로 쓰지 말 것" in prompt


def test_없으면_그_줄이_아예_없다():
    assert "이맘때 미리 해 두는 것" not in 밭()
